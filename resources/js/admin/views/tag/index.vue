<template>
    <div>
        <a-page-header style="border: 1px solid rgb(235, 237, 240)" title="Tag List"
        ...
        <a-modal
                :title="dialogType==='edit'?'Edit Tag':'New Tag'"
                :visible="visible"
                @ok="handleOk"
...
<script>
    ...
    export default {
        ...
        methods: {
            getList() {
                this.loading = true
                request.get('/tags', {params: {page: this.pagination.current, page_num: this.pagination.pageSize}}).then(result => {
                    this.loading = false
            ...
            doSubmit() {
                ...
                if (isEdit) {
                    request.put('/tags/'+this.currentData.id, this.form.getFieldsValue()).then(result => this.callback(result))
                } else {
                    request.post('/tags', this.form.getFieldsValue()).then(result => this.callback(result))
                }
            },
            callback(result) {
            ...
            handleDelete(id) {
                request.delete('/tags/'+id).then(result => this.callback(result))
            },
            handleCancel(e) {
